	...
	struct mmc_ioc_device_power_control  __user *ic_ptr)
{
	...
	struct mmc_blk_data *md;
	struct mmc_card *card;
//<2016/06/30-stevenchen, MTK patch ALPS02778150 to fix MTBF issue
	struct mmc_host *host;
//>2016/06/30-stevenchen
	int err=0;
...
//<2016/06/30-stevenchen, MTK patch ALPS02778150 to fix MTBF issue
	...
	if (host->ops->device_power_control) {
		err = host->ops->device_power_control(host, idata->flag);
		if (err == MMC_POWER_CONTROL_FAIL) {
			pr_err("[SD_DEBUG]Power Control: %s: error %d POWER %s Faild, because of other reasons\n",
				mmc_hostname(host), err, idata->flag == MMC_DEVICE_POWER_ON ? "ON" : "OFF");
		...
		err = MMC_POWER_CONTROL_FAIL;
		pr_err("[SD_DEBUG]Power Control: %s: error %d POWER %s Faild, device_power_control is NULL\n",
			mmc_hostname(host), err, idata->flag == MMC_DEVICE_POWER_ON ? "ON" : "OFF");
	}
