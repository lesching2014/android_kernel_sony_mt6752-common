...
//<<<2014/11/25-youchihwang, for factory test
#define TestFlashlightOnADBcommand
//>>>2014/11/25-youchihwang, for factory test

//<2014/10/29-stevenchen, Enable MiraVision in HW_PDP1 and fix build error
#if !defined(HW_PDP1)
//#define TestFlashlightOnADBcommand
#endif
//>2014/10/29-stevenchen

//# << 2014/08/26-youchihwang, It cause camera on android to reboot so need undefine it after testing
//# adb sys/class/
#ifdef TestFlashlightOnADBcommand
//# >> 2014/08/26-youchihwang, It cause camera on android to reboot so need undefine it after testing
//# adb sys/class/

//# << 2014/08/21-42962-youchihwang, [i53] create file to test lm3642 flash led driver.
#include <linux/ctype.h>
#include <linux/i2c.h>
#include <mach/upmu_common_sw.h>

//# << 2014/10/21-youchihwang, modify for factory front flash led test
...
#define GPIO_CAMERA_CHIP_EN GPIO125
//# << 2014/08/21-42962-youchihwang, [i53] create file to test lm3642 flash led driver.
#define GPIO_CAMERA_FLEN1 GPIO124
//#define GPIO_CAMERA_FLASH_EN_M_GPIO  GPIO_MODE_00
#define GPIO_CAMERA_FLEN2 GPIO131
...
// [Lavender][FlashLED][akenhsu] Add way to custom Front / Rear Flash current for Camera Team Calibration 20150225 BEGIN
extern int RT4505_read_reg(struct i2c_client *client, u8 reg);

#define FRONT_FLASH_CURR_ADDR	0x01
#define REAR_FLASH_CURR_ADDR	0x00
#define FLASH_LED_CTRL_ADDR	0x04

#define FLASH_ON 1
#define FLASH_OFF 0

#define ALL_LED_OFF	0x00
#define FRONT_FLASH_ON	0x20
#define REAR_FLASH_ON	0x02

// [Lavender][FlashLED][akenhsu] Modify the current of Front FlashLED on Lavender to 300mAh 20150330 BEGIN
#if defined(COSMOS)
u8 g_front_flash_level = 0x0A;
#else // Lavender
u8 g_front_flash_level = 0x01; // 300mAh
#endif
// [Lavender][FlashLED][akenhsu] 20150330 END
u8 g_rear_flash_level = 0x0D;

static void switch_ic(u8 is_on)
{
	msleep(10);
	if (FLASH_ON == is_on) {
		mt_set_gpio_mode(GPIO_CAMERA_CHIP_EN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_CHIP_EN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_CHIP_EN, GPIO_OUT_ONE);
	} else {
		mt_set_gpio_mode(GPIO_CAMERA_CHIP_EN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_CHIP_EN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_CHIP_EN, GPIO_OUT_ZERO);
	}
	msleep(10);
}
static void switch_front(u8 is_on)
{
	msleep(10);
	if (FLASH_ON == is_on) {
		mt_set_gpio_mode(GPIO_CAMERA_FLEN2, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_FLEN2, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_FLEN2, GPIO_OUT_ONE);
	} else {
		mt_set_gpio_mode(GPIO_CAMERA_FLEN2, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_FLEN2, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_FLEN2, GPIO_OUT_ZERO);
	}
	msleep(10);
}
static void switch_rear(u8 is_on)
{
	msleep(10);
	if (FLASH_ON == is_on) {
		mt_set_gpio_mode(GPIO_CAMERA_FLEN1, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_FLEN1, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_FLEN1, GPIO_OUT_ONE);
	} else {
		mt_set_gpio_mode(GPIO_CAMERA_FLEN1, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_CAMERA_FLEN1, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CAMERA_FLEN1, GPIO_OUT_ZERO);
	}
	msleep(10);
}
// [Lavender][FlashLED][akenhsu] 20150225 END
...
//	return sprintf(buf, "%u\n", ret);
	...
	databuf[0]=0x04;
	databuf[1]=0x02;
	RT4505_write_reg(RT4505_i2c_client, databuf[0], databuf[1]);
...
// [Lavender][FlashLED][akenhsu] Add way to custom Front/Rear Flash current for Camera Team Calibration 20150225 BEGIN
static ssize_t save_front_flash_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u8 input_value;
	u8 read_value;
	...
	printk("[flashlight] %s, input value=%d, write to IC. \n", __func__, input_value);
	g_front_flash_level = input_value;

	switch_ic(FLASH_ON);
	RT4505_write_reg(RT4505_i2c_client, FRONT_FLASH_CURR_ADDR, input_value);
	msleep(10);
	read_value = RT4505_read_reg(RT4505_i2c_client, FRONT_FLASH_CURR_ADDR);
	printk("[flashlight] %s, read value=%d from IC \n", __func__, read_value);

	switch_front(FLASH_ON);
	RT4505_write_reg(RT4505_i2c_client, FLASH_LED_CTRL_ADDR, FRONT_FLASH_ON);
	msleep(600);
	RT4505_write_reg(RT4505_i2c_client, FLASH_LED_CTRL_ADDR, ALL_LED_OFF);
	switch_front(FLASH_OFF);

	switch_ic(FLASH_OFF);
	return count;
}

static ssize_t save_rear_flash_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u8 input_value;
	u8 read_value;
	...
	printk("[flashlight] %s, input value=%d, write to IC. \n", __func__, input_value);
	g_rear_flash_level = input_value;

	switch_ic(FLASH_ON);
	RT4505_write_reg(RT4505_i2c_client, REAR_FLASH_CURR_ADDR, input_value);
	msleep(10);
	read_value = RT4505_read_reg(RT4505_i2c_client, REAR_FLASH_CURR_ADDR);
	printk("[flashlight] %s, read value=%d from IC \n", __func__, read_value);

	switch_rear(FLASH_ON);
	RT4505_write_reg(RT4505_i2c_client, FLASH_LED_CTRL_ADDR, REAR_FLASH_ON);
	msleep(600);
	RT4505_write_reg(RT4505_i2c_client, FLASH_LED_CTRL_ADDR, ALL_LED_OFF);
	switch_rear(FLASH_OFF);
...
// [Lavender][FlashLED][akenhsu] Add way to custom Front/Rear Flash current for Camera Team Calibration 20150130 BEGIN
static DEVICE_ATTR(front_flash_level, 0664, NULL, save_front_flash_level);
static DEVICE_ATTR(rear_flash_level, 0664, NULL, save_rear_flash_level);
...
// [Lavender][FlashLED][akenhsu] Add way to custom Front/Rear Flash current for Camera Team Calibration 20150225 BEGIN
	, &dev_attr_front_flash_level
	, &dev_attr_rear_flash_level
// [Lavender][FlashLED][akenhsu] 20150225 END
};
...
//static struct i2c_board_info __initdata i2c_lm3642={ I2C_BOARD_INFO("lm3642", (0x37>>1))};////init in module
