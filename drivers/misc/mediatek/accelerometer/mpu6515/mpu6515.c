...
static void gsensor_irq_work(struct work_struct *work)
{
	...
	if ((err = MPU6515_ReadCalibrationEx(mpu6515_i2c_client, cali, raw)))    /*offset will be updated in obj->offset*/
	{
		GSE_ERR("read offset fail, %d\n", err);
	} else {
		p_cust_data = (MPU6515_CUST_DATA *)data.set_cust_req.custData;
		data.set_cust_req.sensorType = ID_ACCELEROMETER;
		data.set_cust_req.action = SENSOR_HUB_SET_CUST;
		p_cust_data->setCali.action = MPU6515_CUST_ACTION_SET_CALI;
		p_cust_data->setCali.data[MPU6515_AXIS_X] = cali[MPU6515_AXIS_X];
		p_cust_data->setCali.data[MPU6515_AXIS_Y] = cali[MPU6515_AXIS_Y];
		p_cust_data->setCali.data[MPU6515_AXIS_Z] = cali[MPU6515_AXIS_Z];
