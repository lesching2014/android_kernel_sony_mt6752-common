...
static uint32_t sensor_delay = 0;
...
static void gsensor_irq_work(struct work_struct *work)
{
    ...
    if ((err = BMA250_ReadCalibrationEx(bma250_i2c_client, cali, raw)))    /*offset will be updated in obj->offset*/
    {
        GSE_ERR("read offset fail, %d\n", err);
    } else {
        p_cust_data = (BMA250_CUST_DATA *)data.set_cust_req.custData;
        data.set_cust_req.sensorType = ID_ACCELEROMETER;
        data.set_cust_req.action = SENSOR_HUB_SET_CUST;
        p_cust_data->setCali.action = BMA250_CUST_ACTION_SET_CALI;
        p_cust_data->setCali.data[BMA250_AXIS_X] = cali[BMA250_AXIS_X];
        p_cust_data->setCali.data[BMA250_AXIS_Y] = cali[BMA250_AXIS_Y];
        p_cust_data->setCali.data[BMA250_AXIS_Z] = cali[BMA250_AXIS_Z];
        len = offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, custData) + sizeof(p_cust_data->setCali);
        SCP_sensorHub_req_send(&data, &len, 1);
    }
    ...
    if (enable_status) {
        ...
        req.set_delay_req.delay = sensor_delay;
        len = sizeof(req.activate_req);
        err = SCP_sensorHub_req_send(&req, &len, 1);
        if (err) {
            GSE_ERR("Set sensor delay fail!\n");
        }
    }
...
static long compat_bma250_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	GSE_FUN();
	...
	case COMPAT_GSENSOR_IOCTL_INIT:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_INIT, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_READ_CHIPINFO:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_READ_CHIPINFO, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_READ_GAIN:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_READ_GAIN, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_READ_RAW_DATA:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_READ_RAW_DATA, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_READ_SENSORDATA, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_SET_CALI:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_SET_CALI, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_CLR_CALI:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_CLR_CALI, (unsigned long)compat_ptr(arg));
	case COMPAT_GSENSOR_IOCTL_GET_CALI:
		return filp->f_op->unlocked_ioctl(filp, GSENSOR_IOCTL_GET_CALI, (unsigned long)compat_ptr(arg));
	default:{
...
/*----------------------------------------------------------------------------*/
//20160525, RickLiu, M sensor on Android M
...
static struct file_operations bma250_fops = {
    ...
    .unlocked_ioctl = bma250_unlocked_ioctl,
#if IS_ENABLED(CONFIG_COMPAT)
    .compat_ioctl = compat_bma250_unlocked_ioctl,
#endif
};
//20160525, RickLiu, M sensor on Android M
