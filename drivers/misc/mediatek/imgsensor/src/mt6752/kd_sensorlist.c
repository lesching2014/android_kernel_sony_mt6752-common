...
//<2016/01/26-zihweishen add Camera Key function for i53-M project begin
//<2014/09/24-kylechang, Add Camera Key(AF:GPIO138/EINT53; Capture:GPIO36/EINT1) 
/*kpd.h file path: ALPS/mediatek/kernel/include/linux */
#include <linux/kpd.h>
//>2014/09/24-kylechang
//>2016/01/26-zihweishen end
...
//<2014/11/19-louisliu, Add Camera Driver IMX214/IMX241
    ...
    #define SUPPORT_I2C_BUS_NUM1       3// Reference=0 , PDP1:Main camera using I2C 3
#endif
#ifndef SUPPORT_I2C_BUS_NUM2
    #define SUPPORT_I2C_BUS_NUM2       1// Reference=2 , PDP1:Sub camera using I2C 1
#endif
//>2014/11/19-louisliu
...
//<2016/01/26-zihweishen add Camera Key function for i53-M project begin
//<2014/09/24-kylechang, Add Camera Key(AF:GPIO138/EINT53; Capture:GPIO36/EINT1) 
static void cam_eint1_interrupt_handler(void)
{
    printk("+++KYLE+++ CamKey  interrupt EINT1  has been triggered\n");
	if (!kpd_input_dev) {
		printk("KPD input device not ready\n");
		return;
	}
#if 1
	unsigned int cam_int1_num=1;
	if (keyCamera == 0){
		// mt_eint_registration(cam_int1_num, TRIGGER_RISING, cam_eint1_interrupt_handler, 1);
		mt_eint_set_polarity(cam_int1_num,MT_EINT_POL_POS);
		input_report_key(kpd_input_dev, 212, 1); //KEY_CAMERA:212, press=1; KEY_HP:211=FOCUS
		input_sync(kpd_input_dev);
		keyCamera = 1;
	}else{
		  //mt_eint_registration(cam_int1_num, TRIGGER_FALLING, cam_eint1_interrupt_handler, 1);
		mt_eint_set_polarity(cam_int1_num,MT_EINT_POL_NEG);
...
static void cam_eint53_interrupt_handler(void)
{
    printk("+++KYLE+++ CamKey  interrupt EINT53  has been triggered\n");
	if (!kpd_input_dev) {
		printk("KPD input device not ready\n");
		return;
	}
#if 1
	unsigned int cam_int53_num=53;
	if (keyFocus== 0){
		mt_eint_set_polarity(cam_int53_num,MT_EINT_POL_POS);
		input_report_key(kpd_input_dev, 211, 1); //Volume UP:115, press=1
		input_sync(kpd_input_dev);
		keyFocus = 1;
	}else{
		mt_eint_set_polarity(cam_int53_num,MT_EINT_POL_NEG);
...
static int CAMERA_HW_probe(struct platform_device *pdev)
{
	...
	*/			 
	 mt_eint_set_sens(cam_int1_num, edge_sensitive);
	 mt_eint_set_hw_debounce(cam_int1_num,10);
	 // [Lavender/Cosmos][Keypad][akenhsu] Fix Focus key will keep down even after hands released 20150317 BEGIN
	 // mt_eint_registration(cam_int1_num, TRIGGER_FALLING, cam_eint1_interrupt_handler, 1);
	 mt_eint_registration(cam_int1_num, TRIGGER_LOW, cam_eint1_interrupt_handler, 1);
	...
	*/
	mt_eint_set_sens(cam_int53_num, edge_sensitive);
	mt_eint_set_hw_debounce(cam_int53_num, 10);
	// [Lavender/Cosmos][Keypad][akenhsu] Fix Focus key will keep down even after hands released 20150317 BEGIN
	// mt_eint_registration(cam_int1_num, TRIGGER_FALLING, cam_eint1_interrupt_handler, 1);
	mt_eint_registration(cam_int53_num, TRIGGER_LOW, cam_eint53_interrupt_handler, 1);
	// [Lavender/Cosmos][Keypad][akenhsu] 20150317 END
	//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_LOW, tpd_eint_interrupt_handler, 1);					
	mt_eint_unmask(cam_int53_num);
	mdelay(10);
...
//<2016/01/26-zihweishen add Camera Key function for i53-M project begin
	...
	//<2015/01/27-kylechang, Remove unused kd_camera_hw_bus2(I2C error INFO)
    //i2c_register_board_info(SUPPORT_I2C_BUS_NUM2, &i2c_devs2, 1);//kyle
	//>2015/01/27-kylechang
/*
