...
#include "imx214Fmipiraw_Sensor.h"

#define PFX "IMX214F_camera_sensor"
...
#if 0
#define IMX214F_OTP_SIZE 1388
#define IMX214F_LSC_addr  512  //0x0200
u8 IMX214F_OTPData[IMX214F_OTP_SIZE];

extern  int iReadData_F(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata);
#endif
//[Lavender]Midify the eFlash driver 20150502 Person E

kal_bool Is_longExp2=0;

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = IMX214F_SENSOR_ID,
	
	.checksum_value =0xc15d2913,
	
	.pre = {
//Person        .pclk = 320000000,              //record different mode's pclk
    .pclk = 240384000,              //record different mode's pclk
		.linelength = 5008,				//record different mode's linelength
//Person        .framelength = 2110,         //record different mode's framelength
	...
	.normal_video = {
//Person		.pclk = 480000000,
		.pclk = 240384000,
		.linelength = 5008,
//Person				.framelength = 3160,
		.framelength = 1600,
		.startx = 0,
		.starty = 0,
...
static void write_shutter(kal_uint32 shutter)
{
	...
	kal_uint32 long_time=0; 
        ...
        kal_uint16 Base_FrameLength1=0xbb33;//0.5s
          kal_uint32 Base_FrameLength2=0x17666;//1s
         kal_uint16 Multi=0;  
    //printk("nyg shutter %d\n",shutter);
	spin_lock(&imgsensor_drv_lock);
			 ...
			 case 32://0.5*64	
			  ...
			  default://1s
				...
				write_cmos_sensor(0x3028, 1);
			  	write_cmos_sensor(0x0202, (Base_FrameLength1 >>8) & 0xFF);
				write_cmos_sensor(0x0203, Base_FrameLength1& 0xFF);
	                   LOG_INF("ERR::long exp is not even number!");
			  	break;
		   	}
          Is_longExp2=0;
...
//<2015/8/25-kylechang, Fix Video HDR AE locked issue(ALPS02232372) by Joseph for MR1
...
// 20150609 DPC down setting for band noise S
write_cmos_sensor(0x30b3,0x01); 
write_cmos_sensor(0x9B00,0x17);
write_cmos_sensor(0x9B01,0x17);
write_cmos_sensor(0x9B02,0x17);
write_cmos_sensor(0x9B03,0x0F);
write_cmos_sensor(0x9B04,0x0F);
write_cmos_sensor(0x9B05,0x0F);
write_cmos_sensor(0x9B06,0x05);
write_cmos_sensor(0x9B07,0x60);
write_cmos_sensor(0x9B08,0x60);
//20150609 DPC down setting for band noise E
...
write_cmos_sensor(0x0340,0x06);  // 20150609 for band noise verification                    
write_cmos_sensor(0x0341,0x40);  // 20150609 for band noise verification
...
//write_cmos_sensor(0x0307,0x44);   // 20150609 for band noise verification                      
write_cmos_sensor(0x0307,0x4C);   // 20150610 for band noise verification 
write_cmos_sensor(0x0309,0x0A);
write_cmos_sensor(0x030B,0x01);
write_cmos_sensor(0x0310,0x00);

//write_cmos_sensor(0x0820,0x0C); 
//write_cmos_sensor(0x0820,0x08); // 20150609 for band noise verification                      
write_cmos_sensor(0x0820,0x09); // 20150610 for band noise verification 
...
write_cmos_sensor(0x3013,0x01);   // 20150609 for band noise verification    
                           
//write_cmos_sensor(0x0202,0x08);                       
//write_cmos_sensor(0x0203,0x34); 
write_cmos_sensor(0x0202,0x06);   // 20150609 for band noise verification 
write_cmos_sensor(0x0203,0x36);   // 20150609 for band noise verification                       
...
static void preview_setting_HDR(void)
{
    ...
    //<2015/4/15-PersonLiu, Tuning rigister 0x0222(EXPO_RATIO) easier (value=1,2,4,8,16)
...
write_cmos_sensor(0x0100,0x01);
    //<2015/4/15-PersonLiu, Tuning rigister 0x0222(EXPO_RATIO) easier (value=1,2,4,8,16)
    LOG_INF("preview_setting_mHDR mode 0x0220(0x01), Ratio 0x0222(0x02)\n");
    //>2015/4/15-PersonLiu
}	/*	preview_setting  */
...
write_cmos_sensor(0xAE20,0x04); 
write_cmos_sensor(0xAE21,0x5C);
//[Cameara] fix capture band noise 20150610 S
write_cmos_sensor(0x30b3,0x01);
write_cmos_sensor(0x9B00,0x17);
write_cmos_sensor(0x9B01,0x17);
write_cmos_sensor(0x9B02,0x17);
...
write_cmos_sensor(0x9B07,0x60);
write_cmos_sensor(0x9B08,0x60);
//[Cameara] fix capture band noise 20150610 E
write_cmos_sensor(0x0138,0x01);
write_cmos_sensor(0x0100,0x01);
}
}
...
//[Cameara] fix capture band noise 20150610 S
	write_cmos_sensor(0x6603,0x01);
	write_cmos_sensor(0x9D00,0x17);
	write_cmos_sensor(0x9D01,0x17);
	write_cmos_sensor(0x9D02,0x17);
	write_cmos_sensor(0x9D03,0x0F);
	write_cmos_sensor(0x9D04,0x0F);
	write_cmos_sensor(0x9D05,0x0F);
	write_cmos_sensor(0x9D06,0x05);
	write_cmos_sensor(0x9D07,0x60);
	write_cmos_sensor(0x9D08,0x60);    
//[Cameara] fix capture band noise 20150610 E    
    write_cmos_sensor(0x0138,0x01);
    write_cmos_sensor(0x0100,0x01);
			...
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
								printk("get imx214F ID, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
//				iReadData_F(IMX214F_LSC_addr,IMX214F_OTP_SIZE,IMX214F_OTPData);//[Lavender]Midify the eFlash driver 20150502 Person
				return ERROR_NONE;
			}	
...
//<2014/11/21-louisliu, Add Camera Driver IMX214B/IMX214F
...
//kin0603
UINT32 IMX214F_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
