...
struct SCP_sensorHub_data {
	...
	struct work_struct      sensor_init_work;
...
struct workqueue_struct *SCP_workqueue = NULL;
static uint32_t SCP_sensor_status;
static int pedometer_delay;
static int activity_delay;
...
/*----------------------------------------------------------------------------*/
static void SCP_sensor_init_work(struct work_struct *work)
{
	int i;
	struct SCP_sensorHub_data *obj = obj_data;
	...
	for (i = 0; i <= ID_SENSOR_MAX_HANDLE; i++) {
		if ((SCP_sensor_status & (1 << i)) != 0) {
			SCP_sensor_enable(i, 1);
			if ((i == ID_ACTIVITY) && (activity_delay != 0))
				SCP_sensor_set_delay(i, activity_delay);
			else if ((i == ID_PEDOMETER) && (pedometer_delay != 0))
				SCP_sensor_set_delay(i, pedometer_delay);
		}
	}
	first_init_done = 0;
}
...
static void SCP_sensorHub_IPI_handler(int id, void *data, unsigned int len)
{
    ...
    else
    {
        switch(rsp->rsp.action)
        {
            ...
            case SENSOR_HUB_SET_CUST:
                wake_up_req = true;
				if ((sensor_status & (1 << rsp->rsp.sensorType)) == 0) {
					sensor_status |= (1 << rsp->rsp.sensorType);
					if (((sensor_status & (1 << ID_ACCELEROMETER)) != 0) &&
						((sensor_status & (1 << ID_PROXIMITY)) != 0)) {
						queue_work(SCP_workqueue, &obj->sensor_init_work);
					}
				}
                break;
        ...
        if(atomic_read(&(obj_data->wait_rsp)) == 1 && true == wake_up_req)
        {
            if (NULL == userData || NULL == userDataLen)
            {
				SCP_ERR("SCP_sensorHub_IPI_handler null pointer, sensorType = %d, action = %d\n",
					rsp->rsp.sensorType, rsp->rsp.action);
            }
            else
            {
...
#endif //#ifdef CONFIG_CUSTOM_KERNEL_GLANCE_GESTURE_SENSOR
	SCP_workqueue = create_singlethread_workqueue("SCP_sensor_init");
	if (!SCP_workqueue)
		SCP_ERR("create_singlethread_workqueue failed\n");
	INIT_WORK(&obj->sensor_init_work, SCP_sensor_init_work);
...
static int SCP_sensor_enable(int sensorType, int en)
{
	...
	if (en)
		SCP_sensor_status |= (1 << sensorType);
	else
		SCP_sensor_status &= ~(1 << sensorType);
