...
//<2014/07/28-Yuting Shih. [Jessicatseng] Integrate backlight IC LM3533.
#if defined( BACKLIGHT_IC_LM3533 )
  #include "leds_lm3533.h"
#endif
//>2014/07/28-Yuting Shih.
// >>> 2016/02/04-dangyiwang, porting lm3533 from cosmos/lavender L to M
	...
	*/	
	...
	switch (cust->mode) {
		...
		if (strcmp(cust->name, "lcd-backlight") == 0) {
			...
			return ((cust_set_brightness)(cust->data))(level);
// <<< 2016/02/04-dangyiwang, porting lm3533 from cosmos/lavender L to M
//<2014/07/25-Yuting Shih. [Jessicatseng-Pelican] Integrate backlight IC LM3533.
#if defined( BACKLIGHT_IC_LM3533 )
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if(led_data->level != level)
	{
		...
		if(strcmp(led_data->cust.name,"lcd-backlight") != 0)
		{
			LEDS_DEBUG("[LED]Set NLED directly %d at time %lu\n",led_data->level,jiffies);
                        // <<< 2016/02/04-dangyiwang, porting lm3533 from cosmos/lavender L to M
						//<2015/06/05-youchihwang, Fix white light doesn't blink after reboot.
			     ...
			     unsigned long *delay_off)
{
  ...
  // <<< 2016/02/04-dangyiwang, porting lm3533 from cosmos/lavender L to M
  //<2014/07/31-Yuting Shih. [Jessicatseng-Pelican] Modify R/G/B LED's level can be controlled by android
  	#if defined( BACKLIGHT_IC_LM3533 )
...
// <<< 2016/02/04-dangyiwang, porting lm3533 from cosmos/lavender L to M
//<2014/07/31-Yuting Shih. [Jessicatseng-Pelican] Modify R/G/B LED's level can be controlled by android
//<2014/07/31-Yuting Shih. [Jessicatseng-Pelican] Integrate backlight IC LM3533 for PRE-MP SW
		#if defined( BACKLIGHT_IC_LM3533 )
