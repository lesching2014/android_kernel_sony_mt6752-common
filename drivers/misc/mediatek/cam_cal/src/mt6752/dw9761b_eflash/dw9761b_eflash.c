...
u8 IMX214B_OTPData[1388];
int imx214b_otp_flag=0;
u8 IMX214B_Date[3];//[Lavender]Add to check the AF Module version 20150711 Person Liu
u8 IMX214B_AWB_UNIT[4];
u8 IMX214B_AWB_GOLD[4];
u8 IMX214B_AF_MACRO[2];
u8 IMX214B_AF_INFINITY[2];
int imx214b_awb_flag=0;
int imx214b_af_flag=0;
...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0xB0>>1)};//A0 for page0 A2 for page 2 and so on for 8 pages
...
static int iReadCAM_CAL(u16 a_u2Addr, u8 * a_puBuff)
{
    ...
    spin_lock(&g_CAM_CALLock); //for SMP
      g_pstI2Cclient->addr = DW9761B_DEVICE_ID>>1;
...
{	
	...
	if (imx214b_af_flag == 0x3)
	{
		if(address == 0x11)
		{
			for(i=0;i<buffersize;i++){
				iBuffer[i]=IMX214B_AF_INFINITY[i];
		...
		if(address == 0x11)
		{
			for(i=0;i<buffersize;i++)
			{
				iReadCAM_CAL((address+i),&readbuff);
				*(iBuffer+i)=readbuff;
				IMX214B_AF_INFINITY[i] = iBuffer[i];				
...
static kal_uint8 Read_AWB_Otp(kal_uint8 address,unsigned char *iBuffer,unsigned int buffersize)
{
	...
	if (imx214b_awb_flag == 0x3)
	{
		if(address == 0x09)
		{
			for(i=0;i<buffersize;i++){
				iBuffer[i]=IMX214B_AWB_UNIT[i];
		...
		else
		{
			for(i=0;i<buffersize;i++)
			{
				iReadCAM_CAL((address+i),&readbuff);
				*(iBuffer+i)=readbuff;
				IMX214B_AWB_GOLD[i] = iBuffer[i];				
				CAM_CALDB("read awb gold[%d]=0x%x\n",i,iBuffer[i]);
			}
			imx214b_awb_flag |= 1<<1;
		}
	}
	return KAL_TRUE;
}
...
static kal_bool Read_LSC_Otp(kal_uint8 address,unsigned char *iBuffer,unsigned int buffersize)
 ...
 if(imx214b_otp_flag){
 	CAM_CALDB("get lsc data Start \n");
	for(i=0;i<buffersize;i++){
		iBuffer[i]=IMX214B_OTPData[i];
...
static void Read_Version(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
		...
		for(i=0;i<buffersize;i++){
			iBuffer[i]=IMX214B_Date[i];
		}
		printk("Read_Version = %d %d %d",IMX214B_Date[0],IMX214B_Date[1],IMX214B_Date[2]);
}
...
//Burst Read Data  iReadData(0x00,932,OTPData);
 int iReadData_B(kal_uint16 ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
//[Lavender]Midify the eFlash driver 20150502 Person S
    CAM_CALDB("[CAMERA SENSOR] iReadData_B offset = %d length = %d\n",ui4_offset,ui4_length);	
#if 0
	if(ui4_length ==1)
    {	
...
static long dw9761b_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	  CAM_CALDB("[CAMERA SENSOR] dw9761b_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
#endif
{
    ...
    switch(a_u4Command)
    {
	    ...
	    //imx214b_otp_flag=1;[Lavender]Midify the eFlash driver 20150502 Person S
            i4RetValue = iReadData_B((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pu1Params);
...
//[Lavender]Add to check the AF Module version 20150711 Person Liu S
    ...
    IMX214B_Date[0] = tempdata[0];
    IMX214B_Date[1] = tempdata[1];
    IMX214B_Date[2] = tempdata[2];
    printk("[AF Module Date] Year = %d,Month = %d, Day = %d \n",IMX214B_Date[0],IMX214B_Date[1],IMX214B_Date[2]);	
//[Lavender]Add to check the AF Module version 20150711 Person Liu E
    return 0;
}
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = dw9761b_Ioctl_Compat,
#endif
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "DW9761B_CAM_CALdrv");
