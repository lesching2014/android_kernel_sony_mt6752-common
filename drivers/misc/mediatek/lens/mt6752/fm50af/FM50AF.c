...
// <<< 2016/03/09-dangyiwang, Proting Camera AF Driver FM50AF from cosmos/lavender L to M
//<2014/11/21-louisliu, Add AF Driver FM50AF
//static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO(AF_DRVNAME, I2C_REGISTER_ID)};
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO(AF_DRVNAME, 0x18)}; //Write ID=0x18, I2C addr=0x0c
//>2014/11/21-louisliu
// >>> 2016/03/09-dangyiwang, Proting Camera AF Driver FM50AF from cosmos/lavender L to M
...
//<2014/11/21-louisliu, Add AF Driver FM50AF
    ...
    *a_pu2Result = ((((u16)pBuff[0]) << 8)&0x0300 + pBuff[1] );
//>2014/11/21-louisliu
// >>> 2016/03/09-dangyiwang, Proting Camera AF Driver FM50AF from cosmos/lavender L to M
...
//<2014/11/21-louisliu, Add AF Driver FM50AF
//Kyle Test
static int s4AF_ReadReg_Test(unsigned short *a_pu2Result)
{
    int  i4RetValue = 0;
    char pBuff[2];
    ...
    if (i4RetValue < 0)
    {
        printk("+++++ s4AF_ReadReg_Test Write Fail \n");
        return -1;
    }
    ...
    if (i4RetValue < 0)
    {
        printk("+++++s4AF_ReadReg_Test Read Fail \n");
        return -1;
    }

    //*a_pu2Result = (((u16)pBuff[0]) << 4) + (pBuff[1] >> 4); //Kyle
    *a_pu2Result = ((((u16)pBuff[0]) << 8)&0x0300 + pBuff[1] );

	printk("+++++s4AF_ReadReg_Test Result pBuff[0]=%d, pBuff[1]=%d\n", (int)pBuff[0], (int)pBuff[1]);
...
//<2014/11/21-louisliu, Add AF Driver FM50AF
    //Kye Test
	unsigned short InitPos11;

	ret = s4AF_ReadReg_Test(&InitPos11);
	if(ret == 0)
	{
		printk("+++s4AF_ReadReg_Test Result= %6d \n", InitPos11);
