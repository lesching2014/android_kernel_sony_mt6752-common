    ...
    switch(host->id){
        case 1:
            if (on && host->mmc && host->mmc->over_current_card) {
                pr_err("over current card, bypass power on\n");
                return;
            }
...
static void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
    ...
    if(host->mmc->card){
        ...
        pr_err("[SD_DEBUG]card is not added, cannot be removed.\n");
    }
...
//<2014/11/15-stevenchen, [All] Create proc/emmc_vendor to distinguish emmc flash.
static struct proc_dir_entry *proc_emmc_vendor;
//>2014/11/15-stevenchen
...
static int proc_emmc_vendor_show(struct seq_file *m, void *v)
{
	...
	{
	    /* KMR310001M_B611 RPMB:0x20*128KB = 4096KB, 
	       KMRNW0001M_B509 RPMB:0x04*128KB = 512KB 	*/
	    if(emmc_rpmb_size == 0x20)
		seq_printf(m, "Samsung KMR310001M_B611,16GB+16Gb \n");         
	    else
		seq_printf(m, "Samsung KMRNW0001M_B509,8GB+16Gb \n");         			
	}
	else
	{
...
static int __init mt_msdc_init(void)
{
    ...
    //<2014/11/15-stevenchen, [All] Create proc/emmc_vendor to distinguish emmc flash.
    if ((proc_emmc_vendor = proc_create( "emmc_vendor", 0, NULL, &proc_emmc_vendor_fops)));
    //>2014/11/15-stevenchen
#endif /* CONFIG_MTK_EMMC_SUPPORT && CONFIG_PROC_FS */
