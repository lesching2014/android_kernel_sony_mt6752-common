...
//[Arima_Lavender][RaymondLin]Fix record video with camera is broken when play the video begin
    params->dsi.PLL_CLOCK = 520; //Vender suggestion 
...
int   vRet = 0;
    ...
    if( LCM_ENABLE == enable )
    {
        ...
        cmd   = 0x00; /* AVDD */
		//[Arima_Lavender][RaymondLin] Set AVDD to 5V begin
        data  = 0x0A; //0x0E; /* 0x0E: +5.4V, 0x0F: +5.5V ,0x0A: +5.0V */    
		//[Arima_Lavender][RaymondLin] Set AVDD to 5V end
        ...
        cmd   = 0x01; /* AVEE */
		//[Arima_Lavender][RaymondLin] Set AVEE to -5V begin
        data  = 0x0A; //0x0E; /* 0x0E: -5.4V, 0x0F: -5.5V ,0x0A: -5.0V*/
		//[Arima_Lavender][RaymondLin] Set AVEE to -5V end
        vRet = lcm_gate_write_bytes( cmd, data );
      #if defined( BUILD_LK )
        if( vRet )
          dprintf( 0, "[LK]%s: lcm_gate set cmd = 0x%02X error\n", __func__, cmd );
...
{
    printk("[Raymond][Kernel]%s: Otm1906a LCM resume begin \n", __func__ );
    lcm_init_innolux();
	printk("[Raymond][Kernel]%s: Otm1906a LCM resume end \n", __func__ );
}
...
//[Arima_Lavender][RaymondLin] Modify LCM power on sequence begin
    if( LCM_PANEL_SOURCE == lcm_id_gpio && id == LCM_OTM1906A_ID ) /* Innolux otm1906a 6" */
	{
	vRet=1;
	SET_RESET_PIN( 0 );
    MDELAY( 3 );
