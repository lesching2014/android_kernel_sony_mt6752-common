...
******************************************************************************/
  ...
  **==========================================================*/
  ...
  static struct LCM_setting_table lcm_initialization_setting[] =
  {
    ...
    { 0x55, 1, { 0x00 }},
    { 0x5E, 1, { 0x00 }},
//<2014/07/31-Yuting Shih. Add for screen rotation 180 degree.
    { 0x36, 1, { 0xC0 }},  // MY(Bit7)=1,MX(Bit6)=1,ML(Bit4)=0,MH(Bit2)=0 : 11000000
  //{ 0x36, 1, { 0xD4 }},  // MY(Bit7)=1,MX(Bit6)=1,ML(Bit4)=1,MH(Bit2)=1 : 11010100
    { REGFLAG_UDELAY, 5, {}},
//>2014/07/31-Yuting Shih.
  //////////////////////////////////////////////////////////////////////////////////////
...
//<2014/08/15-Yuting Shih. Add for power manager.
/**********************************************************
** power manager.
***********************************************************/
#define   LCM_ENABLE          1
...
static void lcm_power_enable(int enable)
{
#if !defined( FPGA_EARLY_PORTING )
...
**
***********************************************************/
static void lcm_gate_power(int enable)
{
#if !defined( FPGA_EARLY_PORTING )
#if defined( BUILD_LK )
...
int   vRet = 0;
    ...
    if( LCM_ENABLE == enable )
    {
      ...
      #if defined( BUILD_LK )
        if( vRet )
          dprintf( 0, "[LK]nt35595--lcm_gate-- cmd = 0x%02X -- i2c write error --\n",cmd );
        else
          dprintf( 0, "[LK]nt35595--lcm_gate-- cmd = 0x%02X -- i2c write success --\n",cmd );
      #else
        if( vRet < 0 )
          printk("[KERNEL]nt35595--lcm_gate-- cmd = 0x%02X -- i2c write error--\n", cmd );
        else
          printk("[KERNEL]nt35595--lcm_gate-- cmd = 0x%02X -- i2c write success--\n", cmd );
      #endif
...
{
    lcm_power_enable( LCM_ENABLE );
}

static void lcm_suspend_power(void)
{
    lcm_power_enable( LCM_DISABLE );
}
...
#if defined( BUILD_LK )
    dprintf( 0, "%s, LK nt35595 debug: nt35595 id = 0x%04X\n", __func__, id );
#else
    printk("%s, kernel nt35595 horse debug: nt35595 id = 0x%04X\n", __func__, id );
#endif
