...
//[Arima_Lavender][RaymondLin] implement Truly LCM gamma value 20150618 end
   ...
   //[Arima_Lavender][RaymondLin] implement Truly LCM color tuning 20150917 begin
{ 0xCA, 43,{ 0x1D ,0xFC ,0xFC ,0xFC ,0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00 ,0x00 , 0x00 ,0x00 ,0x00}}, 
//[Arima_Lavender][RaymondLin] implement Truly LCM color tuning 20150917 end
//[Arima_Lavender][RaymondLin] Vender suggestion due to fix flicker issue begin
...
static void lcm_power_enable_truly_r63422(int enable)
{
#if !defined( FPGA_EARLY_PORTING )
...
{
    lcm_power_enable_truly_r63422( LCM_ENABLE );
}

static void lcm_suspend_power_truly_r63422(void)
{
    lcm_power_enable_truly_r63422( LCM_DISABLE );
}
...
static unsigned int lcm_compare_id_truly_r63422(void)
{
	...
	if( LCM_PANEL_SOURCE == lcm_id_gpio ) /* Truly */
	{
	  if( id == LCM_ID_R63422 ) /* Truly R63422 */
	  	{
	         #if defined( BUILD_LK )
                  dprintf( 0, "[LK][Raymond]: LCM module is Truly r63422\n");
                #else
                  printk("[Kernel][Raymond]: LCM module is Truly r63422\n");
                #endif
	          vRet=1;
	  	}
    }
