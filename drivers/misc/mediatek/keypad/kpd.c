...
//<2016/01/26-zihweishen add HALL_sensor function for i53-M project begin
//<2015/02/20-stevenchen, Smart cover implementation
//<2014/11/14-stevenchen, [All] HALL_sensor_implementation
#if CUST_EINT_HALL_1_NUM
#define HALL_POLARITY  	( CUST_EINT_HALL_1_TYPE )

//<2015/02/26-stevenchen, Power key will not enable when LID closed.
enum LID_STATE{
	LID_OPEN,
	LID_CLOSE,
};
//>2015/02/26-stevenchen

static void hall_handler(unsigned long data);
static DECLARE_TASKLET(hall_tasklet, hall_handler, 0);
static u8 hall_state = ( HALL_POLARITY == CUST_EINTF_TRIGGER_FALLING || HALL_POLARITY == CUST_EINTF_TRIGGER_LOW)? 1: 0;
//<2015/02/26-stevenchen, Power key will not enable when LID closed.
static bool lid_state = LID_OPEN;
//>2015/02/26-stevenchen
#endif
//>2014/11/14-stevenchen
//>2015/02/20-stevenchen
...
static enum hrtimer_restart aee_timer_func(struct hrtimer *timer)
{
	...
	//aee_trigger_kdb();    //removed by champwang, 20160708, requested by sony.
...
static void mt_hall_eint_register(void)
{
	mt_eint_registration(CUST_EINT_HALL_1_NUM, HALL_POLARITY,
	                         hall_eint_handler, false);
}
...
static void hall_handler(unsigned long data)
{
	...
	//<2015/02/26-stevenchen, Power key will not enable when LID closed.
	lid_state = hall_state?LID_CLOSE:LID_OPEN;
	printk( "[Steven] Lid %s \n", (lid_state==LID_CLOSE)?"close":"open");
	//>2015/02/26-stevenchen

	hall_state=!hall_state;

	/* for SW_LID, 0: lid open => slid, 1: lid shut => closed */
	input_report_switch(kpd_input_dev, SW_LID, !hall_state);
	input_sync(kpd_input_dev);
...
{
//<2015/02/26-stevenchen, Power key will not enable when LID closed.
#ifdef CUST_EINT_HALL_1_NUM
	if(lid_state==LID_CLOSE)
	{
