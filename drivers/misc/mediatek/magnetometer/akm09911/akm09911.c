...
//<2014/09/26-JackHu, proc_create function
...
//>2014/12/12-Jack, Proting M sensor AKM09911 (32bit daemon) for Android L
...
// [L][I2C][akenhsu] Modify I2C speed to 400KHz of M-Sensor on Lavender 20150410 BEGIN
#if defined(LAVENDER)
		this_client->timing = 400;
#endif // LAVENDER
// [L][I2C][akenhsu] 20150410 END
...
static long akm09911_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	//<2014/09/26-JackHu, fixed the apk can't get sensor, root cause is daemon path issue 
	char layout;
	//<2014/09/26-JackHu
	struct i2c_client *client = this_client;  
	struct akm09911_i2c_data *data = i2c_get_clientdata(client);
	...
	switch(cmd)
	{
		...
		case COMPAT_ECS_IOCTL_RESET:
		 ...
		 case COMPAT_ECS_IOCTL_READ:
			...
			if(ret)
			{
				AKMDBG("ECS_IOCTL_READ unlocked_ioctl failed.\n");
				return ret;
			}			 
...
static int proc_msensor_open(struct inode *inode, struct file *file)
{
    return single_open(file, tscpu_read_log,  inode->i_private);
}
static const struct file_operations msensor_fops = {
    .open  = proc_msensor_open,
    .read  = seq_read,//ecompass_status_read_proc,
	.llseek  = seq_lseek,
...
// [L][I2C][akenhsu] 20150410 END
	...
	//>2014/12/12-Jack, Proting M sensor AKM09911 (32bit daemon) for Android L
	unsigned int GPIO_SET=(GPIO79 | 0x80000000);
	printk(KERN_ERR " AKM09911 akm09911_probe: befor init prob Jack Setting\n");
	//<2014/09/26-JackHu, Fixed the i2c address and ID error
	mt_set_gpio_mode( GPIO_SET, GPIO_MODE_00 );  
    mt_set_gpio_dir( GPIO_SET, GPIO_DIR_OUT );
    mt_set_gpio_out( GPIO_SET, GPIO_OUT_ONE );
	udelay(5);
	mt_set_gpio_out( GPIO_SET, GPIO_OUT_ZERO );
	udelay(5);
	mt_set_gpio_out( GPIO_SET, GPIO_OUT_ONE );
	udelay(300);
	//<2014/09/26-JackHu
...
static int __init akm09911_init(void)
{
	...
	proc_create("ecompass_status",  0, NULL, &msensor_fops);
#if 0
