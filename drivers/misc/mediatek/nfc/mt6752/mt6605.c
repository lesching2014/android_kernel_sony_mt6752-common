...
static struct i2c_board_info nfc_board_info __initdata = { I2C_BOARD_INFO(I2C_ID_NAME, I2C_NFC_SLAVE_7_BIT_ADDR) };
...
static void mt6605_dev_irq_handler(void);
...
#ifdef CONFIG_64BIT
	I2CDMAWriteBuf =
	    (char *)dma_alloc_coherent(&client->dev, MAX_BUFFER_SIZE, (dma_addr_t *) &I2CDMAWriteBuf_pa, GFP_KERNEL);
#else
	I2CDMAWriteBuf =
	    (char *)dma_alloc_coherent(NULL, MAX_BUFFER_SIZE, (dma_addr_t *) &I2CDMAWriteBuf_pa, GFP_KERNEL);
#endif
...
#ifdef CONFIG_64BIT
	I2CDMAReadBuf =
	    (char *)dma_alloc_coherent(&client->dev, MAX_BUFFER_SIZE, (dma_addr_t *) &I2CDMAReadBuf_pa, GFP_KERNEL);
#else
	I2CDMAReadBuf =
	    (char *)dma_alloc_coherent(NULL, MAX_BUFFER_SIZE, (dma_addr_t *) &I2CDMAReadBuf_pa, GFP_KERNEL);
#endif
...
static ssize_t mt6605_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	...
	while (1) {
		...
		ret_tmp = i2c_master_send(mt6605_dev->client, (unsigned char *)(uintptr_t)
	...
	/* pr_debug(" % s : writing % zu bytes.Status % zu \ n ", __func__, count_ori,ret); */
	return ret;
}
