...
#if 1   /* spinlock */ //[I53_M][zihweishen] ALPS02761676 MTK provide solution to solve DMS07727119 KE crash 2016/6/17
...
static int set_cur_volt_extbuck(struct mt_cpu_dvfs *p, unsigned int volt) /* volt: vproc (mv*100) */
{
    ...
    if (volt > cur_vproc) {
        ...
        do {
            ...
            if (unlikely(!((cur_vsram > cur_vproc) && (MAX_DIFF_VSRAM_VPROC >= (cur_vsram - cur_vproc))))) {
                // <<< 2016/06/16-youchihwang, DMS07726524. Print log for Observing
				unsigned int i, val;
				//---------------------add in 20160615--------------------------------//
				// read extbuck chip id to verify I2C is still worked or not
				for (i=0; i<10; i++) {
					val = ((mt6311_get_cid() << 8) | (mt6311_get_swcid()));
					cpufreq_err("add by mtk read chip id from I2C, id: 0x%x\n", val);
				}

				// read pmic wrap chip id
				for (i=0; i<10; i++) {
					pwrap_read(0x200, &val);
					cpufreq_err("add by mtk pmic wrap CID = %x\n", val);
				}
				//---------------------add in 20160615 end--------------------------------//
                // >>> 2016/06/16-youchihwang, DMS07726524. Print log for Observing
                dump_opp_table(p);
