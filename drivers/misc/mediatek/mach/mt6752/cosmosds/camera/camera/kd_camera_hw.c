...
// <<< 2016/03/07-dangyiwang, Proting Camera Driver IMX214B/IMX214F from cosmos/lavender L to M
//[Lavender]Add Main imx214B/Sub imx214F Camera driver Person Liu 20150116 S                     
#if defined(LAVENDER)
                        //for sub sensor
                     {  CAMERA_CMRST1_PIN,
...
#endif
//[Lavender]Add Main imx214B/Sub imx214F Camera driver Person Liu 20150116 E 
// >>> 2016/03/07-dangyiwang, Proting Camera Driver IMX214B/IMX214F from cosmos/lavender L to M
                   };
...
// <<< 2016/03/07-dangyiwang, Proting Camera Driver IMX214B/IMX214F from cosmos/lavender L to M
#if defined(LAVENDER)
		{SENSOR_DRVNAME_IMX214B_MIPI_RAW,
			{
		...
		{SENSOR_DRVNAME_IMX214F_MIPI_RAW,
			{
				{SensorMCLK,Vol_High, 0},
				{DOVDD, Vol_1800, 0},
				{AVDD,  Vol_2800, 0},
...
// >>> 2016/03/07-dangyiwang, Proting Camera Driver IMX214B/IMX214F from cosmos/lavender L to M
#else	
// <<< 2016/03/07-dangyiwang, Proting Camera Driver IMX214/IMX241 from cosmos/lavender L to M
		...
		{SENSOR_DRVNAME_IMX241_MIPI_RAW,
			{
				{SensorMCLK,Vol_High, 0},
				{AVDD,	Vol_2800, 0},
				{DVDD,	Vol_1200, 0},
...
//[Lavender]Add Main imx214B/Sub imx214F Camera driver Person Liu 20150116 S                     
    ...
    if(pwInfo.PowerType == AVDD)
    {
		...
		if(pinSetIdx == 1)
		{
			...
			if(TRUE != hwPowerOn(PMIC_APP_SUB_CAMERA_POWER_A,pwInfo.Voltage,mode_name))
			{
    ...
    else if(pwInfo.PowerType == DVDD)
    {
		...
		if(pinSetIdx == 1)
		{
			PK_DBG("[CAMERA SENSOR] Sub camera GPIO VCAM_D power on");
			if(mt_set_gpio_mode(GPIO_CAMERA_LDO_EN_PIN,GPIO_MODE_GPIO)){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
			if(mt_set_gpio_dir(GPIO_CAMERA_LDO_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}                        
			if(mt_set_gpio_out(GPIO_CAMERA_LDO_EN_PIN,Vol_High)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
		}   
		else
		{
...
//[Lavender]Add Main imx214B/Sub imx214F Camera driver Person Liu 20150116 S                     
    ...
    if(pwInfo.PowerType == AVDD)
    {
		...
		if(pinSetIdx == 1)
		{
			PK_DBG("[CAMERA SENSOR] Sub camera VCAM_A power down");
    ...
    else if(pwInfo.PowerType == DVDD)
    {
        if(PowerCustList.PowerCustInfo[1].Gpio_Pin == GPIO_UNSUPPORTED)
        {
            if(pinSetIdx==1)
            {
	            if(mt_set_gpio_mode(GPIO_CAMERA_LDO_EN_PIN,GPIO_MODE_GPIO)){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
       	     if(mt_set_gpio_dir(GPIO_CAMERA_LDO_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}                        
	            if(mt_set_gpio_out(GPIO_CAMERA_LDO_EN_PIN,Vol_Low)){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
            }
