...
static s32 i2c_set_speed(mt_i2c *i2c)
{
	...
	if ((mode == FS_MODE && khz > MAX_HS_MODE_SPEED)
	    || (mode == HS_MODE && khz > MAX_HS_MODE_SPEED)) {
		I2CERR(" the speed is too fast for this mode.\n");
		I2C_BUG_ON((mode == FS_MODE && khz > MAX_HS_MODE_SPEED)
			   || (mode == HS_MODE && khz > MAX_HS_MODE_SPEED));
		ret = -EINVAL_I2C;
		goto end;
	}
...
void _i2c_dump_info(mt_i2c *i2c)
{
	...
	if (i2c->dma_en) {
		...
		I2CLOG("INFRACFG: 0x180: 0x%x, 0x184: 0x%x, 0x188: 0x%x, 0x18C: 0x%x, 0x190: 0x%x\n",
		     __raw_readl(i2c_infracfg_base + 0x180), __raw_readl(i2c_infracfg_base + 0x184),
		     __raw_readl(i2c_infracfg_base + 0x188), __raw_readl(i2c_infracfg_base + 0x18C),
...
static s32 _i2c_deal_result(mt_i2c *i2c)
{
	...
	int dma_err = 0;
//[I53_M][zihweishen] ALPS02744824 MTK provide temp patch to solve DMS07726258 KE crash begin 2016/6/13	
	char fifo_debug[16]= {0};
	int i=0;
	int temp_len=0;
	int len_err_flag =0;
//[I53_M][zihweishen] end 2016/6/13
	/* I2CFUC(); */
...
//[I53_M][zihweishen] end 2016/6/13
		...
		i2c_writel(i2c, OFFSET_INTR_STAT, (I2C_HS_NACKERR | I2C_ACKERR | I2C_TRANSAC_COMP));
		//20160331-JackHu, I2C DMA content reset (ALPS02622136)
		if (i2c->dma_en) //add
            dma_reset(i2c); //add
		//20160331-JackHu, I2C DMA content reset
		/*Clear fifo address */
...
#define I2C_GPIOS_DATA_NUM 2
	...
	if (i2c->id != I2C2) {
		if (of_property_read_u32_array
		    (pdev->dev.of_node, "mtk,scl-gpio", scl_gpios, I2C_GPIOS_DATA_NUM)) {
			I2CERR("I2C get scl-gpio failed\n");
			return -ENODEV;
		}

		if (of_property_read_u32_array
		    (pdev->dev.of_node, "mtk,sda-gpio", sda_gpios, I2C_GPIOS_DATA_NUM)) {
			I2CERR("I2C get sda-gpio failed\n");
			return -ENODEV;
		}

		I2CLOG("scl_gpio: %d %d, sda_gpio: %d %d\n", scl_gpios[0], scl_gpios[1],
			sda_gpios[0], sda_gpios[1]);
	...
	if (i2c->id != I2C2) {
		ret =
		    request_irq(i2c->irqnr, mt_i2c_irq, IRQF_TRIGGER_NONE, dev_name(&pdev->dev),
				i2c);
		if (ret) {
