...
{
    //<2015/09/25-JackHu, Avoid the buffer be modified to cause Kernel panic.
...
// [L][I2C][akenhsu] Modify I2C speed of Charger IC to 400KHz 20150408 BEGIN
#if defined(LAVENDER)
    new_client->timing = 400; // AKenHsu I2C 400KHz
#endif // LAVENDER
...
kal_uint32 fan5405_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    battery_xlog_printk(BAT_LOG_FULL,"---R---: Reg 0x%X, Mask 0x%X, Shift 0x%X\n", RegNum, MASK, SHIFT);
...
kal_uint32 fan5405_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //battery_xlog_printk(BAT_LOG_FULL,"--------------------------------------------------\n");
    battery_xlog_printk(BAT_LOG_FULL,"--Cfg--: Reg 0x%X, val 0x%X, Mask 0x%X, Shift 0x%X\n", RegNum, val, MASK, SHIFT);
...
{   
    ...
    battery_xlog_printk(BAT_LOG_FULL,"--reg--: Reg 0x%X, val 0x%X\n", RegNum, val);

    ret = fan5405_write_byte(RegNum, val);
    battery_xlog_printk(BAT_LOG_FULL,"[fan5405_reg_config_interface] write Reg[%X]=0x%X\n", RegNum, val);
...
static ssize_t store_fan5405_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] buf is %s and size is %zu \n",buf,size);
