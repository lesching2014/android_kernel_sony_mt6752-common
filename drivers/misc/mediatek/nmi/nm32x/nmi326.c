...
#include <linux/irq.h>
//#include <asm/irq.h>
//#include <asm/mach/irq.h>	//L remark
...
//#define NMI_RESET_PIN		GPIO122
//#define NMI_IRQN_PIN		GPIO121

#define NMI_RESET_PIN		GPIO_DTV_RST_PIN
#define NMI_IRQN_PIN		GPIO_DTV_EINT_PIN
...
extern int nmi326_spi_get_chip_id(unsigned long *chip_id);
...
#endif
#define NMI_DTV_VDDIO_18 MT6325_POWER_LDO_VCAM_IO
#define NMI_DTV_VDD_12 MT6325_POWER_LDO_VGP2
...
{
        if(TRUE != hwPowerOn(NMI_DTV_VDDIO_18, VOL_1800,NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable VDDIO_18 digital power\n");
        }      

         if(TRUE != hwPowerOn(NMI_DTV_VDD_12, VOL_1200,NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable VDD_12 digital power\n");
        }      
        
	// VDTV_2V8(RF1628) Vdd enable
	//pmic_ldo_vol_sel(MT6325_POWER_LDO_VTCXO1, VOL_2800);
	//pmic_ldo_enable(MT6325_POWER_LDO_VTCXO1, KAL_TRUE);
	hwPowerOn(MT6325_POWER_LDO_VTCXO1, VOL_2800, "NMI_HW_DRVNAME");
...
void isdbt_gpio_power_off(void)
{
	// VDTV_2V8(RF1628) Vdd disable
    	hwPowerDown(MT6325_POWER_LDO_VTCXO1, "NMI_HW_DRVNAME");
	...
	// ANT switch as low
 	mt_set_gpio_out(GPIO_ANT_SW_PIN, 0);
        ...
        //{
        //    NM_KMSG("[ISDBT] Fail to disable VDDIO_18 digital power\n");
        //}

         if(TRUE != hwPowerDown(NMI_DTV_VDD_12, NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to disable VDD_12 digital power\n");
        }      
}
...
void isdbt_irq_handler(void)
{
	...
	printk("[isdbt] isdbt_irq_handler!\n");
...
mt_eint_registration(CUST_EINT_DTV_NUM, EINTF_TRIGGER_FALLING, isdbt_irq_handler, 1);
	...
	printk("[isdbt] isdbt_gpio_interrupt_register!\n");
}
...
void isdbt_gpio_interrupt_unregister(void)
{
	...
	// gpio
	//mt_set_gpio_mode(NMI_IRQN_PIN, GPIO_MODE_GPIO);
	//mt_set_gpio_dir(NMI_IRQN_PIN, GPIO_DIR_IN);
...
static void isdbt_read_gpio()
{
    ...
    val = mt_get_gpio_in(NMI_IRQN_PIN);
    NM_KMSG("~~~~~ <isdbt> val=%d\n", val);
...
static int isdbt_open(struct inode *inode, struct file *filp)
{
	...
	// Thunder added for NMI APK scan issue. 20150505
	isdbt_gpio_power_on();
...
static int isdbt_ioctl(/*struct inode *inode, */struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case IOCTL_ISDBT_CHECK_CHIP_ID:
		{
			unsigned long chip_id = 0x00;
			printk("DTV IOCTL_ISDBT_CHECK_CHIP_ID");
			return (nmi326_spi_get_chip_id(&chip_id));
			//return (0x01);
			//break;
		}
...
static struct platform_device isdbt_driver_devices = {
	.name = "isdbt",
	.id = -1,
};
...
static int __init isdbt_init(void)
{
	...
	else
	{
		...
		NM_KMSG("<isdbt> Check Samba_DS Fail\n");
		pmic_ldo_enable(NMI_DTV_VDD_12, KAL_FALSE);
		return 0;
	}

	NM_KMSG("<isdbt> isdbt_init \n");

	result = platform_device_register(&isdbt_driver_devices);
	if (result)
	{
		printk("isdbt register error \n");
		return result;
	}
	printk("isdbt register\n");
...
//device_initcall_sync(isdbt_init);
module_exit(isdbt_exit);
