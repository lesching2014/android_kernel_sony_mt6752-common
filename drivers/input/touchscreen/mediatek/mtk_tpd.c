...
/*[i53_M][zihweishen] Remove one device name mtk-tpd when use synaptics touch 20160506 begin*/
	//tpd->dev->name = TPD_DEVICE;
/*[i53_M][zihweishen]20160506 end*/	
...
/*[i53_M][zihweishen] Remove one device name mtk-tpd when use synaptics touch 20160506 begin*/
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
			if (tpd_driver_list[i].tpd_device_name == TPD_DEVICE1){
			tpd->dev->name = TPD_SYN_DEVICE;
		}
			else if(tpd_driver_list[i].tpd_device_name == TPD_DEVICE2){
			tpd->dev->name = TPD_DEVICE;
		}

		if ((tpd_driver_list[i].tpd_device_name == TPD_DEVICE2) ||(tpd_driver_list[i].tpd_device_name == TPD_DEVICE1) ) {
