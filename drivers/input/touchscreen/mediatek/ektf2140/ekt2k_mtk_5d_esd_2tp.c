...
//[i53M][zihweishen] Add function for avoid build error begin 2016/01/26
...
/*[i53M][zihweishen] Modify for touch FW of 2'nd source 20160127 begin*/

//<2015/04/09-stevenchen, Fix kernel build error when building open source kernel package
/*[Lavender][bozhi_lin] upgrade touch elan firmware to 0x550B 20150722 begin*/
#if defined(LAVENDER)
static uint8_t file_fw_data_truly[] = {
  #include "I5-3L_FW550B_ID00B9_20150721_SlideTest.i"
};
//static uint8_t file_fw_data_liano[] = {
//  #include "eKTF2150_i5-3_GIS_V5501.i"
//};
static uint8_t *file_fw_data=file_fw_data_truly;
/*[Lavender][bozhi_lin] 20150722 end*/
#else /* For Cosmos */
// Zihwei modify for update touch Firmware 2015 04/15
static uint8_t file_fw_data_truly[] = {
  #include "eKTF2150_i5-3_Truly_V5517.i" /* modify */
};
//static uint8_t file_fw_data_liano[] = {
...
//>2015/04/09-stevenchen
/*[i53M][zihweishen] 20160127 end*/
...
//#else
  static const struct i2c_device_id tpd_i2c_id[] = {{"ektf2k_mtk", 0}, {}};
  static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("ektf2k_mtk", (I2C_ELAN_SLAVE_ADDR>> 1))};
...
//#endif
//Steven end
/*[i53M][zihweishen] Need open when use SM20 project 20160127 begin*/
...
/*[i53M][zihweishen] Modify I53-M project touch driver code function format 20160127 begin*/
...
static void elan_power_enable(int enable)
{
    ...
    #else
      #if 1
        hwPowerOn( MT6325_POWER_LDO_VGP1, VOL_DEFAULT, "TP" );
        printk("[elan] MT6325_POWER_LDO_VGP1: VOL_DEFAULT\n");
      #else
...
#endif
#if 1
/*[Lavender][bozhi_lin] add delay time for improve firmware upgrade failed 20150814 begin*/
#if defined(LAVENDER)
    if( PageNum == iPage ) //20150813, Joanna modify IAP delay time
    {
      mdelay( 500 );
    }
#else
...
#endif
/*[Lavender][bozhi_lin] 20150814 end*/
    else
    {
/*[Lavender][bozhi_lin] add delay time for improve firmware upgrade failed 20150814 begin*/
/*[Lavender][bozhi_lin] For elan touch, add file check to disable firmware auto-upgrade and fine tune firmware auto-upgrade time 20150428 begin*/
#if defined(LAVENDER)
      mdelay(10); //20150813, Joanna modify IAP delay time
#else
      mdelay( 70 );
#endif
/*[Lavender][bozhi_lin] 20150428 end*/
/*[Lavender][bozhi_lin] 20150814 end*/
    }
#endif
...
/*[Lavender][bozhi_lin] add delay time for improve firmware upgrade failed 20150814 begin*/
#if defined(LAVENDER)
      mdelay( 50 ); //20150813, Joanna modify IAP delay time
#else
      mdelay( 70 );
#endif
/*[Lavender][bozhi_lin] 20150814 end*/
...
int ret = 0;
  ...
  if( ret )
  {
    printk( KERN_ERR "[elan]%s: sysfs_create_file (dev_attr_gpio) failed\n", __func__ );
    return ret;
  }
  ...
  if( ret )
  {
    printk( KERN_ERR "[elan]%s: sysfs_create_group (dev_attr_vendor) failed\n", __func__ );
    return ret;
  }
...
//[I53_M][zihweishen] Delete EV_ABS event when touch up lead to touch hover event error for ALPS02737337 begin 2015/10/14
            //input_report_abs( idev, ABS_MT_TOUCH_MAJOR, 0 );
            //input_report_abs( idev, ABS_MT_WIDTH_MAJOR, 0 );
//[I53_M][zihweishen] end 2016/6/14
            input_mt_sync( idev );
            tpd_down_flag = 0;
          } break;
...
//[I53_M]zihweishen Modify report node stuck error begin 2016/02/09
	/*if (gpio_get_value(private_ts->intr_gpio))
	{
		printk("[elan] Detected the jitter on INT pin");
		return IRQ_HANDLED;
	}*/
//[I53_M]zihweishen Modify report node stuck error end 2016/02/09
#if defined( ESD_CHECK )  //0604
  have_interrupts = 1;
#endif
  tpd_flag = 1;
...
//uint8_t cmd[] = {CMD_W_PKT, 0x58, 0x00, 0x01};
  ...
  elan_power_enable( ELAN_ENABLE );
#endif
//zihweishen Modify COSMOS touch resume after, touch not functional for awhile 2016/03/31 begin
