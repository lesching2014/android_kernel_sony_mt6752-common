...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	*/
	...
	for_each_process(tsk) {
		...
		if (!p)
			continue;
//<2016/6/22,alreoryuan.add special handler for don't kill file commander when at low memory			
		if (strstr(p->comm,"systems.fileman")!= NULL)
        {
            task_unlock(p);
           lowmem_print(1, "Don't kill file commander, %d", __LINE__);
           continue;
        }
//>2016/6/22,alreoryuan.	        
